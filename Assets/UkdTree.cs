...
public class UkdTree<T> : IEnumerable<T>, IEnumerable where T : Component
{
    ...
    public UkdTree(bool just2D = false)
    {
        _just2D = just2D;
    }
    ...
    public UkdTree<T> FindAll(Predicate<T> match)
    {
        var list = new UkdTree<T>(_just2D);
